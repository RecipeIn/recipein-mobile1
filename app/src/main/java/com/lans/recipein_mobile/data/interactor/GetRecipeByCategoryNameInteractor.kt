...
class GetRecipeByCategoryNameInteractor @Inject constructor(
    private val recipeRepository: IRecipeRepository,
) : GetRecipeByCategoryNameUseCase {
    override suspend fun invoke(name: String): Flow<Resource<List<Recipe>>> {
        return flow {
            recipeRepository.getAll().collect { result ->
                when (result) {
                    is Resource.Success -> {
                        if (result.data.isNotEmpty()) {
                            emit(Resource.Success(result.data.filter { it.category!!.name == name }))
                        } else {
                            emit(Resource.Empty)
                        }
                    }
